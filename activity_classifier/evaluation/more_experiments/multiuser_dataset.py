...
def process_mmwave(i, f):
    ...
    mmwave_df['User'] = f.split('/')[1].split('.')[0]
...
def read_mmwave():
    mmwave_files = glob.glob('mmWave_data/*debasree*.txt')
    return pd.concat([process_mmwave(i, f) for i,f in enumerate(mmwave_files)], ignore_index=True)
...
def process_activity():
    activity_files = glob.glob('*baseline.csv')
...
mmwave_df['User'].value_counts()


# In[15]:


doppz = np.array(mmwave_df[mmwave_df.User == 'debasree_salma_clapping_squats']['doppz'].values.tolist())


# In[20]:


sns.heatmap(doppz.std(axis=0))
...
merged_df = merged_df[['datetime', 'rangeIdx', 'dopplerIdx', 'numDetectedObj', 'range', 'peakVal', 
                        'x_coord', 'y_coord', 'Activity', 'User', 'Position', 'Orientation', 
                        'activity_class']]
...
x_val = df[df.numDetectedObj == 17].iloc[0:1,:]['x_coord'].values[0]
y_val = df[df.numDetectedObj == 17].iloc[0:1,:]['y_coord'].values[0]
